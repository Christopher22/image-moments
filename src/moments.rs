...
use crate::{
    ...
    Index, Order, Point, Scalar, SupportedIndex, SupportedOrder,
};

/// The raw, spatial movement of an image or contour.
#[derive(Debug, Clone, PartialEq)]
pub struct Moments<T: Scalar, const ORDER: usize>(
    pub(crate) <Order<ORDER> as SealedSupportedOrder<T>>::Storage,
)
where
    Order<ORDER>: SupportedOrder<T>;

impl<T: Scalar, const ORDER: usize> Moments<T, ORDER>
where
    Order<ORDER>: SupportedOrder<T>,
{
    /// Read the moment at a specific position.
    #[inline(always)]
    pub fn get<const I: usize, const J: usize>(&self) -> T
    where
...
impl<'a, T: Scalar, P: 'a + Point<T>, const ORDER: usize> FromIterator<&'a P> for Moments<T, ORDER>
where
    Order<ORDER>: SupportedOrder<T>,
{
...
mod tests {
    ...
    fn test_empty() {
        ...
        let moments: Moments<f64, 3> = points.iter().collect();
